...
export async function GET() {
  try {
    const dataPath = join(
      process.cwd(),
      "public/data/data_statistics/economics/Economic_Statistics_Services.json",
    );
    const raw = await readFile(dataPath, "utf-8");
    const rows: ServicesRow[] = JSON.parse(raw);

    let keys: string[] = [];
    try {
      const objects = await listObjects("data_statistics/economic/Services");
