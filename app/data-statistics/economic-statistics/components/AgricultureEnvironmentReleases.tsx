...
type AgricultureEnvironmentReleasesProps = {
  title?: string;
  subtitle?: string;
};
...
export default function AgricultureEnvironmentReleases({
  title,
  subtitle,
}: AgricultureEnvironmentReleasesProps) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error("Unable to load Agriculture & Environment releases");
        }
        ...
        const mapped: ReleaseItem[] = incoming.map((row: any) => {
          ...
          const yearValue =
            row.year && !Number.isNaN(Number(row.year))
              ? Number(row.year)
              : parsed.year;
          const categoryLabel = String(row.category ?? "").trim() || "Others";
